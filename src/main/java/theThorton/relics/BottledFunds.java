...
import theThorton.utilPatch.BottleField;
import theThorton.utilPatch.TextureLoader;

import java.util.function.Predicate;

import static theThorton.ThortMod.makeRelicOutlinePath;
import static theThorton.ThortMod.makeRelicPath;

public class BottledFunds extends CustomRelic implements CustomBottleRelic, CustomSavable<Integer> {
    ...
    // ID, images, text.
    public static final String ID = ThortMod.makeID("BottledFunds");
    ...
    // I've included that already in patches.relics.BottleField
    // The basemod wiki I linked above has comments about onSave and onLoad

    @Override
    public Predicate<AbstractCard> isOnCard() {
        return BottleField.inBottleField::get;
    }
    ...
    public void onLoad(Integer cardIndex) {
        ...
        if (cardIndex >= 0 && cardIndex < AbstractDungeon.player.masterDeck.group.size()) {
            ...
            if (card != null) {
                BottleField.inBottleField.set(card, true);
    ...
    public void onUnequip() { // 1. On unequip
        if (card != null) { // If the bottled card exists (prevents the game from crashing if we removed the bottled card from our deck for example.)
            ...
            if (cardInDeck != null) {
                BottleField.inBottleField.set(cardInDeck, false); // In our SpireField - set the card to no longer be bottled. (Unbottle it)
            }
        }
    }
    ...
    public void update() {
        ...
        if (!cardSelected && !AbstractDungeon.gridSelectScreen.selectedCards.isEmpty()) {
            ...
            BottleField.inBottleField.set(card, true); // Use our custom spire field to set that card to be bottled.
